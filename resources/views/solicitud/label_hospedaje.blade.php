        ...
        <div class="col-md-8">
            <table class="table">
                <tr>
                    ...
                    <td>
                        <input type="text" name="costo_unitario" id="costo_unitario" class="txt-re-col form-control" placeholder="$" value="0.00">
                    </td>
                    <td>
                        Monto autorizado
                    </td>
...
<script>
    $(document).ready(function(){
        ...
        $('#numero_noches').val($numero_dias);
        $('#numero_cuartos').val(1);
        ...
        $('#numero_cuartos').unbind().bind('blur',function(){
            ...
            if($numeroCuartos != '' && $costoUnitario != '' && $numeroNoches != ''){
                ...
                $('#costo_total').val(parseFloat($precioTotal)).number(true,2);
            }
        });
            ...
            if($submit == 0){
                if(parseFloat($valor) > parseFloat($('#monto_company_policies_id').val())){
                    ...
                    if($reques_id == ''){
                        ...
                        var datas = getFormData($form);
                        window.parent.hospedaje(datas,window.parent.$('#request_id').val());
                        window.parent.blurStuff(0);
                    }
                }
            }else{
        ...
        $('#nacional_cheque').unbind().bind().number(true,2);
        $('#nacional_debito').unbind().bind().number(true,2);
        $('#nacional_credito').unbind().bind().number(true,2);
        $('#nacional_efectivo').unbind().bind().number(true,2);
        $('#nacional_amex').unbind().bind().number(true,2);

        $('#extranjero_cheque').unbind().bind().number(true,2);
        $('#extranjero_debito').unbind().bind().number(true,2);
        $('#extranjero_credito').unbind().bind().number(true,2);
        $('#extranjero_efectivo').unbind().bind().number(true,2);
        $('#extranjero_amex').unbind().bind().number(true,2);
    });

    function total(){
        $numNoches = $('#numero_noches').val();
        $numCuartos = $('#numero_cuartos').val();
        $montoAu =  $('#monto_total').val();
        $totalNo = $montoAu * $numNoches;
        $total = $totalNo * $numCuartos;
        $('#costo_total').val($total).number(true,2);
    }
</script>
@endsection